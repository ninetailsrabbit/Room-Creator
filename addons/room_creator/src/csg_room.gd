...
func create_materials_on_room() -> void:
	if generate_materials:
		var shapes =  RoomCreatorUtils.get_all_children(self).filter(func(child): return child is CSGBox3D)
		var index: int = 0
...
func walls() -> Array[CSGBox3D]:
	...
	for wall: CSGBox3D in RoomCreatorUtils.remove_falsy_values([front_wall, back_wall, right_wall, left_wall]):
		result.append(wall)
